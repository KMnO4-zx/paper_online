ALTER TABLE papers
  ...
  ADD COLUMN IF NOT EXISTS code_evidence TEXT,
...
BEGIN
  ALTER TABLE papers
    ADD CONSTRAINT papers_code_status_check
    CHECK (code_status IN ('open_source', 'unavailable', 'not_found', 'unknown'));
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;

CREATE INDEX IF NOT EXISTS idx_papers_code_status
ON papers(code_status);

DROP FUNCTION IF EXISTS search_papers_optimized(TEXT, TEXT, BOOLEAN, BOOLEAN, BOOLEAN, INT, INT);
DROP FUNCTION IF EXISTS search_papers_optimized(TEXT, TEXT, BOOLEAN, BOOLEAN, BOOLEAN, TEXT, INT, INT);
...
BEGIN
  ...
  keyword_matches AS (
    ...
    FROM keywords k
    JOIN scoped_papers p ON p.id = k.paper_id
    WHERE
      search_keywords
  ...
  matched_papers AS (
    ...
    JOIN scoped_papers p ON p.id = rp.id
  )
  SELECT
    mp.id,
    mp.title,
...
DROP FUNCTION IF EXISTS count_papers_optimized(TEXT, TEXT, BOOLEAN, BOOLEAN, BOOLEAN, TEXT);
