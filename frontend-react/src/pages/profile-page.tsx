...
import { BookMarked, ChevronLeft, Eye, Heart, Loader2 } from 'lucide-react';

import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { PaginationBar } from '@/components/pagination-bar';
import { RichContent } from '@/components/rich-content';
import { fetchMyPapers } from '@/lib/api';
...
const SORTS: Array<{ value: MyPaperSort; label: string }> = [
  ...
  { value: 'liked_first', label: '收藏优先' },
  { value: 'updated_at', label: '最近操作' },
  { value: 'title', label: '标题 A-Z' },
];

function parseFilter(value: string | null): MyPaperFilter {
  return value === 'viewed' || value === 'liked' ? value : 'all';
}

function parseSort(value: string | null): MyPaperSort {
  if (value === 'liked_at' || value === 'liked_first' || value === 'updated_at' || value === 'title') {
    return value;
  }
  return 'viewed_at';
}
...
function PaperHistoryCard({ item }: { item: MarkedPaperItem }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 grid gap-2 text-xs text-[#728095] sm:grid-cols-3">
        ...
        <div className="space-y-2">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-[#728095]">
              {user.email} 的阅读和点赞记录，后续推荐系统会基于这些数据工作。
            </p>
          </div>
        </div>
